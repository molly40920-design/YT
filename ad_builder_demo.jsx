        ...
        <div className="fixed inset-0 z-40 overflow-auto" style={{ background: "#F4F6F9" }}>
          <div className="max-w-[1500px] mx-auto p-6">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-sm font-semibold text-slate-700">廣告系列 <span style={{ color: BRAND }}>1</span>　廣告組 <span style={{ color: BRAND }}>{pvAdSets}</span>　廣告 <span style={{ color: OK }}>{pvAds}</span></div>
            </div>
...
function GoogleFlow({ setPlatform }) {
  const geoG = (n) => ({ name: `地區組${n}`, region: "不限", language: "不限", target: "所在地或興趣（推薦）", exclude: "所在地（推薦）", tag: "" });
  ...
  function geoForm(g, i) {
    ...
    return (<>
      <button className="text-sm mb-4 flex items-center gap-1.5" style={{ color: BRAND }}><Copy size={13} />選擇已有地區組</button>
      <Block><FieldLabel>地區</FieldLabel><Segmented options={["不限", "指定"]} value={g.region} onChange={(v) => set2({ region: v })} /></Block>
      <Block><FieldLabel>語言</FieldLabel><Segmented options={["不限", "指定"]} value={g.language} onChange={(v) => set2({ language: v })} /></Block>
  ...
  function renderStep() {
    switch (step) {
      ...
      case 3: return (<>
        ...
        <Block><FieldLabel req>日預算</FieldLabel><UnitInput value={s.dailyBudget} onChange={(v) => set({ dailyBudget: v })} unit="TWD" /></Block>
        {s.goal === "安裝量" && <Block><FieldLabel req>出價</FieldLabel><Segmented options={["目標每次操作費用", "盡可能提高轉化次數"]} value={s.bidType} onChange={(v) => set({ bidType: v })} />{s.bidType === "目標每次操作費用" && <div className="mt-2"><UnitInput value={s.bidAmount} onChange={(v) => set({ bidAmount: v })} unit="TWD" /></div>}<Hint>選「盡可能提高轉化次數」時隱藏出價金額輸入框。</Hint></Block>}
        {s.goal === "應用內操作次數" && <Block><FieldLabel req>出價</FieldLabel><UnitInput value={s.bidAmount} onChange={(v) => set({ bidAmount: v })} unit="TWD" ph="請輸入" /></Block>}
        {s.goal === "應用內操作價值" && <Block><FieldLabel req>tROAS %</FieldLabel><UnitInput value={s.troas} onChange={(v) => set({ troas: v })} unit="%" ph="請輸入" /><Hint>成效目標＝應用內操作價值時，以 tROAS 取代出價。</Hint></Block>}
                ...
                <tbody>
                  {previewList.slice(0, 200).map((r) => (
                    <tr key={r.id} className="border-t border-slate-100 align-top">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap"><button onClick={() => copyRow(r.id)} style={{ color: BRAND }}>複製</button>　<button onClick={() => delRow(r.id)} style={{ color: BRAND }}>刪除</button></td>
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap text-[13px] leading-relaxed">定向：{r.geo.region}<br />排除：不限</td>
                      <td className="px-4 py-3 text-slate-600 text-[13px]">定向：{r.geo.language}</td>
